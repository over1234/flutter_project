                          ...
                          children: <Widget>[
                            Padding(
                              ...
                              child: IconButton(
                                ...
                                onPressed: () {
                                  Navigator.push(
                                      context, MaterialPageRoute(builder: (BuildContext)=> First()));
                                },
                                //나가는 버튼
                              ),
                            ),
                            ...
                            /*
                               Padding(
                                  padding: const EdgeInsets.only(right: 250.0, top: 40.0),
                              ...
                              Padding(
                                padding: const EdgeInsets.only(left: 160.0, bottom: 0.0),
                                child: RadioListTile(
                                    title: Text('공개',
                                    style: TextStyle(
                                      color: const Color(0xff55CED6),
                              ...
                              Padding(
                                padding: const EdgeInsets.only(left: 160.0, bottom: 30.0),
                                child: RadioListTile(
                                    title: Text('비공개',
                                      style: TextStyle(
                                        color: const Color(0xff55CED6),
                              ...
                              child: RaisedButton(
                                ...
                                onPressed: () async {
                                  if(controller.text != '' ) {
                                    ...
                                    /* final f = FirebaseFirestore.instance;
                                          await f.collection(CHATROOM).doc(now.toString()).set(ChatModel(now, controller.text).toJson(),);
                                           var l = result.docs.map((e) => ChatModel.fromJson(e.data())).toList();
                                          var notifyListeners = notifyListeners(); */
                                    var rName = {'방이름' : controller.text };
                                    final db = FirebaseFirestore.instance;
                                    await db.collection('Roomname').add(rName).then((res)=>{
                                      print('DB등록이 되었습니다'),
                                    });
